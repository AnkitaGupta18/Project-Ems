    ...
    if (data.type === "submit") {
      ...
      if (!nextPage) {
        ...
        if (isTheFormValidated() && sendSubmit) {
          ...
          // Don't submit if form has Stripe or Braintree. The form submits automatically after credit card is tokenized
          var isEditMode = (!!~window.location.href.indexOf("/edit/"));
          var autoSubmit = ["stripe", "braintree"];
          if (!(JotForm.payment && autoSubmit.indexOf(JotForm.payment) > -1 && JotForm.isPaymentSelected() && !isEditMode)) {
